...
include("PARAMETERS-buildupatm.jl")
...
function chemJmat(nthis, inactive, activespecies, inactivespecies, Jrates, Tn, Ti, Te, tup, tdown, tlower, tupper)
    ...
    if in(:H2O, activespecies) && in(:HDO, activespecies)
        ...
        # For some reason, the code throws a "cannot broadcast to dimensions" error if we just remove the indices of the water,
        # so we have to just set them to 0.
...
function get_transport_and_J_rates(n, inactive, activesp, inactivesp)
    ...
    # copy all the Jrates into an external dictionary, because we will eventually want to know them...
    for jsp in Jratelist
        global Jrate_storage[jsp] = n_cur_all[jsp]
    end
...
args = Any["temp", "216", "130", "205", "mean"]#Any[ARGS[i] for i in 1:1:length(ARGS)]
...
elseif make_new_alt_grid=="n"
    ...
    defaultatm = results_dir*"sundials-CO2only/atm_state_t_1.0e14.h5"
...
converge_which = "neutrals"# input("Converging ions, neutrals or both?: ")
...
if converge_which == "neutrals"
    ...
    if use_nonzero_initial_profiles
        ...
        n_initial_guess = get_ncurrent("converged_neutral_atmosphere.h5")
        for nn in new_neutrals
            n_current[nn] = n_initial_guess[nn]
        end
        # initialize the static CO2pl profile
        n_current[:CO2pl] = n_initial_guess[:CO2pl]
    end
elseif converge_which == "ions"
    println("converge ions is not implemented in this script")
elseif converge_which == "both"  # Currently this doesn't really work.
    println("converge both neutrals and ions is not implemented in this script")
else
...
# make a storage dictionary for updated Jrates as the simulation runs.
Jrate_storage = Dict([j=>n_current[j] for j in keys(n_current) if occursin("J", string(j))])
...
crosssection = populate_xsect_dict([T_surf, T_tropo, T_exo])
