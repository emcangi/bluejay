...
function Dcoef(T, ntot, species::Symbol)
    ...
    Tṅ: temperature (K) for neutrals
    ntot: total density at the layer calculated for
    species: whichever species we are calculating for

    ntot: number of molecules (all species) this altitude, returned by n_tot
...
function Da_coef(Tp, species::Symbol, ncur, z)
    #=
    Override to calculate the ambipolar diffusion coefficient for an atmospheric layer.
    For ions, ambipolar diffusion is calculated according to Krasnopolsky 2002 and 
    Schunk & Nagy equation 5.55. Note that Krasnopolsky indexes the neurals with k
    but I don't like that so I have indexed them with n. Ions are indexed with i.
    Units: cm/s

    Tp: Plasma temperature (K)
    species: whichever species we are calculating for
    ...
    try
        ...
        D_a = (kB * Tp) / (mi * sum_nu_in)
        return D_a
    catch
        throw("Argument Error: Failed to pass either ncur or z")
    end
end
...
function fluxcoefs(z, dz, species::Symbol, ncur, controltemps::Array)
    ...
    if charge_type(species) == "neutral"
        Dp = Dcoef(Tp_neutral, ntp, species)
        D0 = Dcoef(T0_neutral, nt0, species)
        Dm = Dcoef(Tm_neutral, ntm, species)

        H0p = scaleH(z+dz, Tp_neutral, ncur)
        H00 = scaleH(z, T0_neutral, ncur)
        H0m = scaleH(z-dz, Tm_neutral, ncur)
    else
        Dp = Da_coef(Tp_plasma, species, ncur, z)
        D0 = Da_coef(T0_plasma, species, ncur, z)
        Dm = Da_coef(Tm_plasma, species, ncur, z)
...
function lower_up(z, dz, species::Symbol, ncur, controltemps::Array)
    ...
    # Total atmospheric layer density
    ntp = n_tot(ncur, z+dz)
    nt0 = n_tot(ncur, z)
    ntm = 1
    ...
    # return the coefficients
    return fluxcoefs(z, dz, [Km, K0, Kp], [Dm, D0, Dp], [Tm_neutral, T0_neutral, Tp_neutral],
                    [Tm_plasma, T0_plasma, Tp_plasma], [Hsm, Hs0, Hsp], [H0m, H00, H0p], species)[2]
end
...
function scaleH(z, T::Float64, mm::Real)
    ...
    =#
    # return kB_MKS*T/(mm*mH*marsM*bigG)*(((z+radiusM)*1e-2)^2)*1e2 # Old MKS
...
function upper_down(z, dz, species::Symbol, ncur, controltemps::Array)
    ...
    # return the coefficients
    return fluxcoefs(z, dz, [Km, K0, Kp], [Dm, D0, Dp], [Tm_neutral, T0_neutral, Tp_neutral],
              [Tm_plasma, T0_plasma, Tp_plasma], [Hsm, Hs0, Hsp], [H0m, H00, H0p], species)[1]
end
...
function populate_xsect_dict(controltemps)#, o3xdata)
    #=
